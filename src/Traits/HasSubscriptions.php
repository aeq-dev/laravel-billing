...
use Bkfdev\Billing\Exceptions\InvalidPlanSubscription;
use Bkfdev\Billing\Models\Plan;
use Bkfdev\Billing\Models\PlanCombination;
use Bkfdev\Billing\Models\PlanSubscription;
use Bkfdev\Billing\Services\SubscriptionPeriod;
use Carbon\Carbon;
...
trait HasSubscriptions
{
    ...
    {
        return $this->morphMany(config('billing.models.plan_subscription'), 'subscriber', 'subscriber_type', 'subscriber_id');
    }
    ...
    public function subscription(?string $subscriptionTag = null)
    {
        ...
        $subscriptionTag = $subscriptionTag ?? config('billing.main_subscription_tag');
     ...
     *
     * @return \Bkfdev\Billing\Models\PlanSubscription|null
     */
    public function subscribedPlans()
    {
        $planIds = $this->subscriptions->reject->isInactive()->pluck('plan_id')->unique();

        return app(config('billing.models.plan'))->whereIn('id', $planIds)->get();
    }
     ...
     * @param \Bkfdev\Billing\Models\Plan|\Bkfdev\Billing\Models\PlanCombination $planCombination Plan pricing and invoice data
    ...
    {
        $tag = $tag ?? config('billing.main_subscription_tag');
