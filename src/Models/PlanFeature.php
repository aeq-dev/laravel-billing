...
class PlanFeature extends Model
{
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->setTable(config('billing.tables.plan_features'));
    }
    ...
    public function getRules(): array
    {
        return [
            'tag' => [
                'required',
                'max:150',
                Rule::unique(config('billing.tables.plan_features'))->where(function ($query) {
                    return $query->where('id', '!=', $this->id)->where('plan_id', $this->plan_id);
                }),
            ],
            'plan_id' => 'required|integer|exists:' . config('billing.tables.plans') . ',id',
