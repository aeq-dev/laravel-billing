...
use Bkfdev\Billing\Exceptions\UsageDenied;
...
use Bkfdev\Billing\Traits\HasGracePeriodUsage;
use Bkfdev\Billing\Traits\HasPricing;
use Bkfdev\Billing\Traits\HasSubscriptionPeriodUsage;
use Bkfdev\Billing\Traits\HasTrialPeriodUsage;
use Bkfdev\Billing\Traits\HasSchedules;
use Carbon\Carbon;
use DB;
...
class PlanSubscription extends Model
{
    ...
    {
        return $this->hasMany(config('billing.models.plan_subscription_feature'), 'plan_subscription_id', 'id');
    }
    ...
    public function usage(): \Illuminate\Database\Eloquent\Relations\HasManyThrough
    {
        return $this->hasManyThrough(
            config('billing.models.plan_subscription_usage'),
            config('billing.models.plan_subscription_feature'),
    ...
    {
        if (!$ignoreFallback && config('billing.fallback_plan_tag')) { // Do not cancel if a fallback plan is set
            $plan = Plan::getByTag(config('billing.fallback_plan_tag'));
            if (!$plan) {
                throw new UnexpectedValueException('Fallback plan ' . config('billing.fallback_plan_tag') . ' does not exist.');
            }
     ...
     * @param int $uses
     *
     * @return \Bkfdev\Billing\Models\PlanSubscriptionUsage|null
     */
    ...
    public function status()
    {
        ...
        if ($this->isOnTrial())
            return 'On Trial';
        if ($this->hasEndedTrial() && !$this->isActive())
            return 'Trial Ended';
        if ($this->isActive())
            return 'Active';
        return 'Active';
